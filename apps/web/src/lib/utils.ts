...
/**
 * Convierte el perfil clínico a texto legible para el prompt de medicamentos
 */
export function profileToText(profile: Record<string, unknown>): string {
  ...
  if (profile.triglycerides_mg_dl) lipidos.push(`Triglicéridos: ${profile.triglycerides_mg_dl} mg/dL`);
  if (lipidos.length > 0) lines.push(`Perfil lipídico — ${lipidos.join(" | ")}`);
  ...
  if (renal.length > 0) lines.push(`Función renal — ${renal.join(" | ")}`);
  ...
  if (Array.isArray(profile.medications) && profile.medications.length > 0)
    lines.push(`Medicación actual: ${profile.medications.join(", ")}`);
