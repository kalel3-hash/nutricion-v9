...
const faqs = [
  ...
  {
    q: "Necesito saber de nutricion o medicina para usar VitalCross AI?",
    a: "No. VitalCross AI esta pensada para que cualquier persona pueda utilizarla, sin necesidad de conocimientos tecnicos o medicos.\n\nLa plataforma traduce informacion clinica y nutricional a un lenguaje claro, accesible y comprensible, para que puedas entender mejor como distintos alimentos se relacionan con tu salud.\n\nEl objetivo es facilitar el acceso a informacion util, no generar confusion ni reemplazar el criterio profesional.",
  },
  {
    q: "Puedo usar VitalCross AI si no tengo analisis de laboratorio recientes?",
    a: "Si. VitalCross AI puede utilizarse incluso si no contas con analisis clinicos recientes.\n\nEn ese caso, los resultados seran mas generales y menos precisos, ya que la personalizacion depende de la informacion disponible.\n\nCuantos mas datos reales y actualizados tengas, mayor sera el nivel de personalizacion del analisis. Contar con estudios clinicos mejora la calidad de la informacion, pero no es un requisito obligatorio para comenzar.",
  },
  {
    q: "La inteligencia artificial de VitalCross AI realiza diagnosticos?",
  ...
  return (
    <div style={{
      ...
      <button onClick={() => setOpen(p => !p)} style={{
        ...
        background: "transparent", border: "none", cursor: "pointer", textAlign: "left", gap: "1rem",
      }}>
      ...
      {open && (
        <div style={{ padding: "0 1.4rem 1.25rem" }}>
          ...
          {a.split("\n\n").map((p, i) => (
            <p key={i} style={{ margin: "0 0 0.75rem", fontSize: "0.9rem", color: "#5F5E5A", lineHeight: 1.75, whiteSpace: "pre-line" }}>{p}</p>
          ))}
        </div>
      )}
    </div>
...
export default function FaqsPage() {
  return (
    <div style={{ minHeight: "100vh", background: "#F0F6FF", fontFamily: "system-ui, sans-serif" }}>
      ...
      <section style={{ background: "linear-gradient(135deg, #0C447C 0%, #185FA5 60%, #378ADD 100%)", padding: "3.5rem 2rem 3rem", textAlign: "center" }}>
        <div style={{ maxWidth: "600px", margin: "0 auto" }}>
          <div style={{ display: "inline-block", background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.25)", borderRadius: "20px", padding: "4px 14px", fontSize: "0.7rem", fontWeight: 700, color: "rgba(255,255,255,0.85)", letterSpacing: "0.8px", marginBottom: "1.25rem", textTransform: "uppercase" }}>
            Centro de ayuda
          </div>
          <h1 style={{ margin: "0 0 1rem", color: "#FFFFFF", fontWeight: 800, fontSize: "clamp(1.6rem, 4vw, 2.5rem)", lineHeight: 1.15 }}>
            Preguntas frecuentes sobre VitalCross AI
          </h1>
          <p style={{ color: "rgba(255,255,255,0.8)", fontSize: "0.95rem", maxWidth: "480px", margin: "0 auto", lineHeight: 1.75 }}>
            En VitalCross AI creemos que tomar decisiones informadas sobre la alimentacion es clave para la salud. Aca respondemos las preguntas mas comunes para que entiendas que hace la plataforma, como funciona y que no hace.
          </p>
        </div>
      </section>
      ...
      <main style={{ maxWidth: "740px", margin: "0 auto", padding: "3rem 1.5rem" }}>
        ...
        <div style={{ background: "linear-gradient(135deg, #0C447C 0%, #185FA5 100%)", borderRadius: "16px", padding: "2.5rem 2rem", textAlign: "center" }}>
          <p style={{ margin: "0 0 0.5rem", fontSize: "1.1rem", fontWeight: 700, color: "#FFFFFF", lineHeight: 1.5 }}>
            VitalCross AI combina tecnologia, datos y conocimiento cientifico.
          </p>
