...
function IconFacebook() {
  return (<svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073C24 5.405 18.627 0 12 0S0 5.405 0 12.073C0 18.1 4.388 23.094 10.125 24v-8.437H7.078v-3.49h3.047V9.41c0-3.025 1.792-4.697 4.533-4.697 1.312 0 2.686.235 2.686.235v2.97h-1.513c-1.491 0-1.956.93-1.956 1.886v2.268h3.328l-.532 3.49h-2.796V24C19.612 23.094 24 18.1 24 12.073z" /></svg>);
}
...
const ss: React.CSSProperties = { display: "flex", alignItems: "center", justifyContent: "center", width: "34px", height: "34px", borderRadius: "8px", background: "#E6F1FB", color: "#185FA5", textDecoration: "none", flexShrink: 0 };

function SE({ href, title, children }: { href: string; title: string; children: React.ReactNode }) {
  return <a href={href} target="_blank" rel="noopener noreferrer" title={title} style={ss} onMouseEnter={e => (e.currentTarget.style.background = "#B5D4F4")} onMouseLeave={e => (e.currentTarget.style.background = "#E6F1FB")}>{children}</a>;
}
function SM({ href, title, children }: { href: string; title: string; children: React.ReactNode }) {
  return <a href={href} title={title} style={ss} onMouseEnter={e => (e.currentTarget.style.background = "#B5D4F4")} onMouseLeave={e => (e.currentTarget.style.background = "#E6F1FB")}>{children}</a>;
}
...
export default function NavbarProtegido() {
  ...
  return (
    <>
      ...
      <nav style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", alignItems: "center", padding: "0.55rem 1.25rem", background: "#FFFFFF", borderBottom: "1px solid #B5D4F4", position: "sticky", top: 0, zIndex: 50, gap: "8px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
          <Link href="/dashboard">
            <Image src="/Logo.png" alt="VitalCross AI" width={62} height={62} style={{ objectFit: "contain", display: "block" }} />
          </Link>
        </div>
